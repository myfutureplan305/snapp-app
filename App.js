...
const SCANS_KEY = "snappy_scans";
const PLAN_KEY = "snappy_plan";
const FREE_LIMIT = 5;
const PLAN_LIMITS = { free: 5, basic: 100, pro: 150 };
...
export default function App() {
  ...
  const pickPhoto = async (fromCamera) => {
    ...
    const result = fromCamera
      ? await ImagePicker.launchCameraAsync({ quality: 0.6, allowsEditing: false, exif: false })
      : await ImagePicker.launchImageLibraryAsync({ quality: 0.6, allowsEditing: false, exif: false });
  ...
  const submit = async (overrideQuery = null) => {
    ...
    if (scansUsed >= limit) { setShowPaywall(true); return; }
    ...
    try {
      ...
      AsyncStorage.setItem(SCANS_KEY, newCount.toString());
    } catch (e) {
  ...
  const PaywallModal = () => (
    <Modal visible={showPaywall} animationType="slide" presentationStyle="pageSheet">
      <View style={[styles.modalBg, { backgroundColor: "#1A1A2E" }]}>
        <SafeAreaView style={{ flex: 1 }}>
          <View style={styles.paywallHeader}>
            <TouchableOpacity onPress={() => setShowPaywall(false)}>
              <Text style={styles.paywallClose}>✕</Text>
            </TouchableOpacity>
          </View>
          <ScrollView contentContainerStyle={styles.paywallContent}>
            <Text style={styles.paywallIcon}>⚡</Text>
            <Text style={styles.paywallTitle}>Unlock Snappy Pro</Text>
            <Text style={styles.paywallSubtitle}>
              {scansUsed >= PLAN_LIMITS[plan]
                ? "You have used all your scans. Upgrade to keep scanning."
                : "Get more scans and unlock all features."}
            </Text>
            <View style={styles.paywallPlans}>
              <TouchableOpacity
                ...
                onPress={() => {
                  setPlan("basic");
                  AsyncStorage.setItem(PLAN_KEY, "basic");
                  setScansUsed(0);
                  AsyncStorage.setItem(SCANS_KEY, "0");
                  setShowPaywall(false);
                  Alert.alert("Subscribed!", "You now have 100 scans per month.");
                }}
                activeOpacity={0.85}
              >
                <View style={styles.planCardLeft}>
                  <Text style={styles.planName}>Basic</Text>
                  <Text style={styles.planDesc}>100 scans per month</Text>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  AsyncStorage.setItem(SCANS_KEY, "0");
                  setShowPaywall(false);
                  Alert.alert("Subscribed!", "You now have 150 scans per month.");
                }}
                activeOpacity={0.85}
              >
                ...
                <View style={styles.planCardLeft}>
                  ...
                  <Text style={[styles.planDesc, { color: "rgba(255,255,255,0.7)" }]}>150 scans per month</Text>
                  <View style={styles.planFeatures}>
                    {["150 photo scans/month", "Find products anywhere", "Save to shopping list", "Search history", "Priority support"].map((f, i) => (
                      <Text key={i} style={[styles.planFeatureText, { color: "rgba(255,255,255,0.9)" }]}>✓  {f}</Text>
                    ))}
                  </View>
                </View>
                <View style={styles.planCardRight}>
                  <Text style={[styles.planPrice, { color: "#fff" }]}>$7.99</Text>
                  <Text style={[styles.planPeriod, { color: "rgba(255,255,255,0.7)" }]}>/month</Text>
                </View>
              </TouchableOpacity>
            </View>
            <Text style={styles.paywallLegal}>Cancel anytime. Billed monthly. Scans reset each billing cycle.</Text>
            <TouchableOpacity onPress={() => setShowPaywall(false)}>
              <Text style={styles.paywallRestore}>Restore purchase</Text>
            </TouchableOpacity>
          </ScrollView>
        </SafeAreaView>
      </View>
      ...
      <TouchableOpacity style={styles.resultCardInner} onPress={() => Linking.openURL(item.link)} activeOpacity={0.75}>
        <View style={styles.resultImgBox}>
          {item.thumbnail
            ...
            : <Text style={{ fontSize: 28 }}>shopping bag</Text>}
        </View>
    ...
    <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.homeContent} showsVerticalScrollIndicator={false}>
      <View style={styles.homeHeader}>
        ...
        <View style={styles.headerRight}>
          ...
          <TouchableOpacity style={styles.scanCountPill} onPress={() => setShowPaywall(true)}>
            <Text style={styles.scanCountText}>{PLAN_LIMITS[plan] - scansUsed} scans left</Text>
          </TouchableOpacity>
        </View>
      </View>
    ...
    <View style={{ flex: 1 }}>
      ...
      {history.length === 0 ? (
        <View style={styles.emptyState}>
          <Text style={styles.emptyEmoji}>clock</Text>
  ...
  const SavedScreen = () => (
    <View style={{ flex: 1 }}>
      ...
      {saved.length === 0 ? (
        <View style={styles.emptyState}>
          <Text style={styles.emptyEmoji}>heart</Text>
          <Text style={styles.emptyTitle}>Nothing saved yet</Text>
          <Text style={styles.emptySubtitle}>Tap Save on any result to add it here</Text>
        </View>
      ) : (
        <FlatList
          data={saved}
            ...
            <GlassCard style={styles.resultCard}>
              <TouchableOpacity style={styles.resultCardInner} onPress={() => Linking.openURL(item.link)} activeOpacity={0.75}>
                <View style={styles.resultImgBox}>
                  {item.thumbnail
                    ? <Image source={{ uri: item.thumbnail }} style={styles.resultImg} resizeMode="contain" />
                    : <Text style={{ fontSize: 28 }}>bag</Text>}
                </View>
                <View style={styles.resultInfo}>
                  <Text style={styles.resultRetailer}>{item.retailer}</Text>
                  <Text style={styles.resultTitle} numberOfLines={2}>{item.title}</Text>
...
const styles = StyleSheet.create({
  ...
  scanCountPill: { backgroundColor: C.accent, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 5 },
  scanCountText: { fontSize: 12, fontWeight: "700", color: "#fff" },
  ...
  photoIcon: { fontSize: 14, fontWeight: "700", color: C.accent },
  ...
  tabIcon: { fontSize: 11, opacity: 0.5, fontWeight: "700" },
  ...
  paywallHeader: { paddingHorizontal: 20, paddingTop: 16, alignItems: "flex-end" },
  paywallClose: { fontSize: 20, color: "rgba(255,255,255,0.5)", padding: 4 },
  paywallContent: { paddingHorizontal: 20, paddingBottom: 40, alignItems: "center" },
  paywallIcon: { fontSize: 52, marginBottom: 16, marginTop: 8 },
  paywallTitle: { fontSize: 28, fontWeight: "800", color: "#fff", textAlign: "center", marginBottom: 10 },
  ...
  planDesc: { fontSize: 13, color: "rgba(255,255,255,0.6)", marginBottom: 12 },
  planFeatures: { gap: 6 },
  planFeatureText: { fontSize: 13, color: "rgba(255,255,255,0.75)", fontWeight: "500" },
  planPrice: { fontSize: 28, fontWeight: "800", color: "#fff" },
  planPeriod: { fontSize: 13, color: "rgba(255,255,255,0.6)" },
  paywallLegal: { fontSize: 12, color: "rgba(255,255,255,0.35)", textAlign: "center", marginTop: 28, lineHeight: 18 },
  paywallRestore: { fontSize: 13, color: "rgba(255,255,255,0.4)", textAlign: "center", marginTop: 12, fontWeight: "500" },
});
